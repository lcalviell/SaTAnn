...
#' @param for_SaTAnn_file REQUIRED - path to the "for_SaTAnn" file containing P_sites positions and junction reads
...
#' @seealso \code{\link{load_annotation}}, \code{\link{forgeBSgenomeDataPkg}}, \code{\link{makeTxDbFromGFF}}, \code{\link{run_SaTAnn}}.
#' @export
...
#' Plot general statistics about SaTAnn results
#'
...
#' @param SaTAnn_output_file Full path to the "_final_SaTAnn_results" RData object output by SaTAnn. See \code{run_SaTAnn}
...
#' @return the function exports a RData object (*SaTAnn_plots_RData) containing data to produce all plots, and produces different QC plots in .pdf format.
...
plot_SaTAnn_results<-function(for_SaTAnn_file,SaTAnn_output_file,annotation_file,coverage_file_plus=NA,coverage_file_minus=NA,output_plots_path=NA,prefix=NA){
    
    
    cat(paste("Plotting SaTAnn results for ",SaTAnn_output_file," ... ",date(),"\n",sep = ""))
    
    if(is.na(prefix)){prefix<-gsub(gsub(basename(SaTAnn_output_file),pattern = "_final_SaTAnn_results",replacement = ""),pattern = "final_SaTAnn_results",replacement = "")}
    
    for (f in c(for_SaTAnn_file,SaTAnn_output_file,annotation_file)){
        if(file.access(f, 0)==-1) {
            stop("The following files don't exist:\n",
                 f, "\n")
        }
    ...
    if(is.na(output_plots_path)){output_plots_path<-paste0(SaTAnn_output_file, "_plots")}
    dir.create(output_plots_path,recursive=TRUE, showWarnings=FALSE)
    
    load(annotation_file)
    load(SaTAnn_output_file)
    ORFs_tx<-SaTAnn_results$ORFs_tx
    ORFs_gen<-SaTAnn_results$ORFs_gen
    selected_txs<-SaTAnn_results$selected_txs
    ORFs_txs_feats<-SaTAnn_results$ORFs_txs_feats
    ORFs_feat<-SaTAnn_results$ORFs_feat
    
    for_SaTAnn<-get(load(for_SaTAnn_file))
    
    list_SaTAnn_plots<-list()
    ...
    list_SaTAnn_plots[["ORFs_found"]]<-orfs_found_n
    ...
    list_SaTAnn_plots[["ORFs_found_pct_tr"]]<-orfs_found_pct
    ...
    list_SaTAnn_plots[["ORFs_found_len"]]<-orfs_found_len
    list_SaTAnn_plots[["ORFs_found_len"]][["pars"]]<-c(14,4)
    ...
    ovv<-findOverlaps(red_ex,for_SaTAnn$P_sites_all)
    aggo<-aggregate(for_SaTAnn$P_sites_all$score[ovv@to],by=list(ovv@from),sum)
    cnts<-aggo[,2]
    ...
    list_SaTAnn_plots[["ORFs_genes"]]<-orfsn_genes
    list_SaTAnn_plots[["ORFs_genes"]][["pars"]]<-c(7,6)
    ...
    list_SaTAnn_plots[["ORFs_genes_tpm"]]<-orfsn_genes_tpm
    list_SaTAnn_plots[["ORFs_genes_tpm"]][["pars"]]<-c(7,6)
    ...
    list_SaTAnn_plots[["ORFs_maxiso"]]<-orfsn_genes_maxiso
    list_SaTAnn_plots[["ORFs_maxiso"]][["pars"]]<-c(7,6)
    ...
    list_SaTAnn_plots[["ORFs_maxiso_tpm"]]<-orfsn_genes_maxiso_tpm
    list_SaTAnn_plots[["ORFs_maxiso_tpm"]][["pars"]]<-c(7,6)
    ...
    list_SaTAnn_plots[["Sel_txs_genes"]]<-sel_txs_genes
    list_SaTAnn_plots[["Sel_txs_genes"]][["pars"]]<-c(7,6)
    ...
    list_SaTAnn_plots[["Sel_txs_genes_tpm"]]<-sel_txs_genes_tpm
    list_SaTAnn_plots[["Sel_txs_genes_tpm"]][["pars"]]<-c(7,6)
    ...
    list_SaTAnn_plots[["Sel_txs_genes_pct"]]<-sel_txs_genes_pct
    list_SaTAnn_plots[["Sel_txs_genes_pct"]][["pars"]]<-c(7,6)
    ...
    b$ps<-assay(summarizeOverlaps(b,reads = for_SaTAnn$P_sites_all,ignore.strand=F,mode="Union",inter.feature=FALSE))
    ...
    list_SaTAnn_plots[["Sel_txs_bins_juns"]]<-selection_bins_juns
    list_SaTAnn_plots[["Sel_txs_bins_juns"]][["pars"]]<-c(5,4.5)
    ...
    if(!is.na(coverage_file_plus) & !is.na(coverage_file_minus)){
        ...
        ORFs_gen<-SaTAnn_results$ORFs_gen
        ORFs_spl_feat_maxORF<-SaTAnn_results$ORFs_spl_feat_maxORF
        
        seqlevels(ORFs_gen)<-seqlevels(for_SaTAnn$P_sites_all)
        seqlengths(ORFs_gen)<-seqlengths(for_SaTAnn$P_sites_all)
        #nope, use coverage
        ...
        all_metapl<-all
        list_SaTAnn_plots[["Meta_splicing_coverage"]]<-all_metapl
        list_SaTAnn_plots[["Meta_splicing_coverage"]][["pars"]]<-c(22,16)
    }
    
    save(list_SaTAnn_plots,file = paste0(output_plots_path,"/",prefix,"_","SaTAnn_plots_RData"))
    for(i in names(list_SaTAnn_plots)){
        pdf(file = paste0(output_plots_path,"/",prefix,"_",i,".pdf"),width = list_SaTAnn_plots[[i]][["pars"]][1] ,height = list_SaTAnn_plots[[i]][["pars"]][2])
        print(list_SaTAnn_plots[[i]])
        dev.off()
    }
    cat(paste("Plotting SaTAnn results for ",SaTAnn_output_file,"  --- Done! ",date(),"\n",sep = ""))
...
#' 
#' @param input_files Character vector with full paths to plot files (*SaTAnn_plots_RData) 
#' generated with \code{plot_SaTAnn_results}. 
...
#' @details This function creates the html report visualizing final SaTAnn results. \cr \cr
#' Input are two lists of the same length: \cr \cr
#' a) \code{input_files}: list of full paths to one or multiple input files 
#' (*SaTAnn_plots_RData files generated with \code{plot_SaTAnn_results}) and \cr \cr
...
#' @seealso \code{\link{plot_SaTAnn_results}}, \code{\link{run_SaTAnn}}
#' @export

create_SaTAnn_html_report <- function(input_files, input_sample_names, output_file){
    ...
    rmd_path <- paste(system.file(package="SaTAnn"),"/rmd/SaTAnn_template.Rmd",sep="")
    
    sink(file = paste(output_file,"_SaTAnn_report_output.txt",sep = ""))
